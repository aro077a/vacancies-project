...
export const TimesheetCard: React.FC<Props> = ({
  ...
  const firstDOW = useMemo(
    () => adminTimesheet?.week && startOfISOWeek(new Date(adminTimesheet.week)),
    [adminTimesheet?.week],
  );
  ...
  return (
    <div className={styles['timesheet-card']}>
      {loadingCandidateTimesheetById ? (
        ...
        <>
          {adminTimesheet?.rows.map((item: TimesheetRows) => {
            const formattedData = {
              ...item,
              id: item.id,
              name: item.name,
            ...
            return (
              <Fragment key={item.id}>
                ...
                <div className={styles['timesheet-card__days']}>
                  {adminTimesheet?.contractType === 'day' &&
          ...
          {adminTimesheet?.additionalExpenses.map((expenses: TimesheetExpenses) => (
