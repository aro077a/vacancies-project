...
import { companiesAndPositionsAsSelectOptionsSelector } from '~/modules/candidateTimesheet/selectors';
...
export const CreateEntryModal: React.FC<CreateEntryModalProps> = memo(function CreateEntryModal({
  ...
  const { createCandidateEntryErrors, creatingCandidateEntry } = useSelector(
    state => state.candidateTimesheet,
  );
  ...
  useEffect(() => {
    if (createCandidateEntryErrors) {
      iterateOverErrorDetailsEachKey<FieldPath<CreateEntryFormValues>>(
        createCandidateEntryErrors,
        (key, value) => {
          setError(key, { type: 'validate', message: value });
        },
      );
    }
  }, [createCandidateEntryErrors, setError]);

  const onSubmit = useCallback(
    (values: CreateTimesheetEntryRequestParams) => {
      dispatch(
        createCandidateEntry.request({
          formValues: values,
          onSuccess: () => {
            onClose();
            openSubmitModal();
            reset();
            dispatch(getCandidateTimesheet.init({ initialFetch: true }));
          },
        }),
      );
    },
        ...
        <div className={styles['modal-entry__container-footer']}>
          ...
          <div className={styles['modal-entry__container-footer-summary']}>
            ...
            <Button
              ...
              loading={creatingCandidateEntry}
            />
          </div>
        </div>
      </div>
