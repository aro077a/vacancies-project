...
import { skipLicensesUpload, uploadCandidateLicenses } from '~/modules/createCandidate/actions';
...
export const UploadLicensesPage: React.FC<RouteComponentProps> = ({ history }) => {
  ...
  const onSubmit = useCallback(() => {
    ...
    if (uploadedFiles.length > 0) {
      dispatch(
        uploadCandidateLicenses.request({
          ...
          onSuccess: () => {
            setUploadedFiles([]);
            history.push(CommonRouter.createCandidate.createCandidatePhoto);
          },
        }),
      );
      return;
    ...
    if (!editMode) {
      dispatch(
        skipLicensesUpload({
          onSkip: () => history.push(CommonRouter.createCandidate.createCandidatePhoto),
        }),
      );
    }
  }, [dispatch, uploadedFiles, editMode, history]);
  ...
  return (
    <div className={styles['page']}>
      {!editMode ? (
        <>
          ...
          <p className={styles['page__subtitle']}>
            We will keep your licenses on file so you don't have to upload it each time. You can
            replace with updated file anytime you wish. You can upload up to 10 licenses
          </p>
        </>
      ) : (
        <div className={styles['page__header-wrapper']}>
      ...
      {uploadedFiles.length > 0 && (
        <div>
          <>
            {uploadedFiles.map((uploadedFile: UploadCandidateLicensesFormValues, index: number) => (
              <div
