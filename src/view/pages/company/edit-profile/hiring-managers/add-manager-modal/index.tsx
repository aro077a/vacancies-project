...
import { addHiringManager, updateHiringManager } from '~/modules/companyEditProfile/actions';
...
export const AddManagerModal: React.FC<AddManagerProps> = memo(function AddManagerModal({
  ...
  const {
    addHiringManagerErrors,
    updateHiringManagerErrors,
    addingHiringManager,
    updatingHiringManager,
  } = useSelector((state: RootState) => state.companyProfile);
  const { hiringManager, editMode } = useSelector(state => state.createEmployer);
  ...
  useEffect(() => {
    if (editMode && hiringManager) {
      const { firstName, lastName, permission, position, office, project, phone, email } =
        hiringManager;
  ...
  }, [addHiringManagerErrors, updateHiringManagerErrors, setError]);
      ...
      >
        <div className={styles['modal-manager__body']}>
          ...
          <Button
            ...
            loading={editMode ? updatingHiringManager : addingHiringManager}
          />
        </div>
      </CenterModal>
      <SuccessModal
